...
// Run only the shim specs in this package's own `e2e` directory (currently just `wait.spec.ts`),
// not the whole shared suite. `devices` comes from this package's own @playwright/test so the shared
// config never loads a second copy of Playwright (older versions throw when two copies are loaded).
